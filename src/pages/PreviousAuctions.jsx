...
export default function PreviousAuctions() {
  ...
  useEffect(() => {
    try {
      ...
      const accountHistory = history.filter(r => {
        const ownerMatch =
          r.ownerAccountId === userId ||
          (userEmail && r.ownerAccountEmail === userEmail);

        if (ownerMatch) {
          return true;
        }

        // Backward compatibility for previously saved records without owner fields.
        return Boolean(r.teams?.some(t => t.id === userId || (userEmail && t.id === userEmail)));
      });

      // Normalise each stored result into the AuctionCard shape
      const normalized = accountHistory.map((r, idx) => {
        // Find the current user's team in the result
        const myTeam = r.teams?.find(t => t.id === userId)
          || r.teams?.[0]
          || null;
